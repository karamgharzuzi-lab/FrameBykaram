...
{ id: "carpet", title: { en: "Red Carpet: Carpet", he: "שטיח אדום: שטיח", ar: "السجادة الحمراء: הסجادة" }, subtitle: { en: "Select carpet color", he: "בחר צבע שטיח", ar: "اختر لون السجادة" } },
...
isSearching: false,
debounce: null,
isAnimating: false // Transition state lock
};
...
<button data-lang="${b.id}" class="px-2 py-1.5 md:px-3 md:py-2 rounded-md md:rounded-xl text-[9px] md:text-sm font-semibold w-full text-center ${state.lang===b.id ? "gold-btn" : "text-white/80 hover:text-white"}">
${b.label}
</button>
`).join("");
...
function goNext() {
if (!isStepValid() || state.isAnimating) return;
